...
class Qlearning:
    ...
    def update_model(self, state, action, reward, next_state):
        ...
        # This is sarsa model(State, Action, Reward, Action, State)
class Sarsa:
	def train(state2, action2, reward2, next_state2):
		Q_sarsa = np.array([state2, action2])
		alpha2 = 0.5
		gamma2 = 1
		formula = Q_sarsa + alpha2 * (reward2 + gamma2 * max(Q_sarsa) - Q_sarsa)
		return formula
